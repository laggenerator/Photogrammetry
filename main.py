...
class MyWindow:
    def __init__(self):
        ...
        self.window.katalogButton.clicked.connect(self.otworz_katalog)
    ...
    def otworz_katalog(self):
        ...
        sciezka_katalogu = os.path.join(glowny_katalog, "work", "zdjecia")
        if sys.platform == 'win32':
            os.startfile(sciezka_katalogu)
        else:
            os.system(f'xdg-open "{sciezka_katalogu}"')
    ...
    def start_colmap(self):
        ...
        ), opcje, self.window.wymusRekonstrukcje.isChecked(), self.window.matchowanieCheckBox.isChecked(), self.window.czyGPU.isChecked())
